...
from braket.v2.schema_common.schema_base import BraketSchemaBase, BraketSchemaHeader
...
class AnnealingTaskResult(BraketSchemaBase):
    ...
    _ANNEALING_TASK_RESULT_HEADER = BraketSchemaHeader(
        name="braket.v2.task_result.annealing_task_result", version="1"
    )
    braketSchemaHeader: BraketSchemaHeader = Field(default=_ANNEALING_TASK_RESULT_HEADER)
    solutions: list[conlist(conint(ge=-1, le=3), min_length=1)] | None = None
    solutionCounts: list[conint(ge=0)] | None = None
    values: list[float] | None = None
    variableCount: conint(ge=0) | None = None
